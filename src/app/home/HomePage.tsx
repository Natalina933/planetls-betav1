  ...
  return (
    <div data-home-heritage className={styles.home}>
      ...
      <section className={styles.hero} aria-labelledby="home-title">
        <Image src="/images/hero-warmv2.jpg" alt="Une maison lumineuse, ouverte sur la verdure" fill priority sizes="100vw" className={styles.heroImage} />
      ...
      <section id="profils" className={styles.profileSection}>
        <div className={styles.section}><p className={styles.eyebrow}>Chacun sa place, un même lieu</p><h2>Une plateforme,<br /><em>trois expériences.</em></h2><div className={styles.profiles}>{copy.profiles.map(profile => <article key={profile.title} id={profile.image}><div className={styles.profileImage}><Image src={`/images/carousel/planetls-private-${profile.image}.png`} alt={profile.title} fill sizes="(max-width: 760px) 100vw, 33vw" /></div><div className={styles.profileBody}><h3>{profile.title}</h3><p>{profile.text}</p><Link href={profile.href} className={styles.textLink}>{profile.action} <span aria-hidden>↗</span></Link></div></article>)}</div></div>
      </section>
      ...
      <footer id="contact" className={styles.footer}><Link href="/home" className={styles.brand}>PlanetLS<small>Simplifiez la location</small></Link><p>Des lieux vivants. Des liens durables.</p><nav aria-label="Liens de pied de page"><Link href="/about">À propos</Link><Link href="/contact">Contact</Link></nav></footer>
    </div>
  );
}
