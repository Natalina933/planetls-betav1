      ...
      <header className={styles.hero}>
        <div className={styles.topline}>
          <span className={styles.eyebrow}>
            <UsersRound size={17} aria-hidden="true" /> Référentiel produit
          </span>
          <DevelopmentSectionNav active="personas" />
        </div>

        <div className={styles.heroContent}>
          <div>
            <h1>Personas PlanetLS</h1>
            <p>
              Les profils de référence qui guident les parcours, les priorités produit et les
              décisions d’expérience utilisateur.
            </p>
          </div>
          ...
          <div className={styles.summary}>
            <article>
              ...
              <span>personas documentés</span>
            </article>
            <article>
              <strong>{validatedCount}</strong>
              <span>validés par l’usage</span>
            </article>
            <article>
              <strong>{personas.length - validatedCount}</strong>
              <span>à confronter au terrain</span>
            </article>
          </div>
        </div>
        ...
        <aside className={styles.governance}>
          ...
          <p>
            <strong>Édition locale.</strong> Tes précisions sont sauvegardées dans ce navigateur.
            Les valeurs initiales du projet restent restaurables à tout moment.
          </p>
          <Button size="sm" variant="outline" onClick={restoreAll} disabled={!storageReady}>
            <RotateCcw size={15} /> Tout restaurer
          </Button>
        </aside>
      </header>
      ...
      <section className={styles.personaGrid} aria-label="Personas produit PlanetLS">
        {personas.map((persona, index) => (
          <PersonaCard
            ...
            persona={persona}
            index={index}
            onEdit={setEditedPersona}
            onRestore={restorePersona}
            onUpdate={savePersona}
          />
        ))}
      </section>
