...
export function buildActivationAlerts(
  ...
  return (["owner", "concierge", "provider"] as const).flatMap((role) => {
    ...
    if (metric.activation_j7_eligible < ACTIVATION_ALERT_POLICY.minimum_eligible) {
      alerts.push({ id: `${role}-sample`, role, severity: "info", kind: "insufficient_data", title: `${ROLE_LABELS[role]} : cohorte insuffisante`, detail: `${metric.activation_j7_eligible}/${ACTIVATION_ALERT_POLICY.minimum_eligible} profils éligibles minimum.`, next_action: "Attendre une cohorte mature avant toute décision d’acquisition." });
      return alerts;
    }
    if (metric.activation_j7 !== null && metric.activation_j7 < policy.target_rate) {
      alerts.push({ id: `${role}-target`, role, severity: metric.activation_j7 < policy.critical_rate ? "danger" : "warning", kind: "below_target", title: `${ROLE_LABELS[role]} : activation sous le seuil`, detail: `${metric.activation_j7}% observé ; cible ${policy.target_rate}% et seuil critique ${policy.critical_rate}%.`, next_action: role === "owner" ? "Auditer la création de la première demande." : role === "concierge" ? "Auditer l’envoi du premier devis." : "Auditer l’accès et l’acceptation de la première mission." });
    }
    const maturePoints = series[role].filter((point) => point.eligible >= ACTIVATION_ALERT_POLICY.minimum_eligible && point.rate !== null).slice(-2);
    if (maturePoints.length === 2) {
      const decline = (maturePoints[0].rate ?? 0) - (maturePoints[1].rate ?? 0);
      if (decline >= ACTIVATION_ALERT_POLICY.decline_points) alerts.push({ id: `${role}-decline`, role, severity: "warning", kind: "declining", title: `${ROLE_LABELS[role]} : tendance en baisse`, detail: `Recul de ${Math.round(decline * 100) / 100} points entre les deux dernières cohortes matures.`, next_action: "Comparer les sources d’acquisition, zones et étapes d’onboarding des deux cohortes." });
    }
    return alerts;
  });
}
