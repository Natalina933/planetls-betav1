...
type Row = {
  ...
  quote_id: string | null; service_request_id: string | null; mission_id: string | null;
};
...
export async function GET(req: NextRequest) {
  try {
    ...
    const { data, error } = await client.from("housing_collaborations")
      .select("id,housing_id,owner_profile_id,concierge_profile_id,quote_id,service_request_id,mission_id")
    ...
    for (const row of rows) {
      ...
      items.push({
        id: row.id, status: "pending_handover",
