  ...
  try {
    ...
    const [{ data: concierge }, { data: owner }] = await Promise.all([
      db
        ...
        .eq("id", invoice.concierge_profile_id)
        .maybeSingle(),
      invoice.owner_profile_id
        ? db
            .from("profiles")
            .select(
    ...
    const rowsHtml = items
      .map(
        ...
        <tr>
          ...
          <td class="num">${formatCurrency(Number(item.unit_price ?? 0), invoice.currency)}</td>
          <td class="num">${formatCurrency(Number(item.line_total ?? 0), invoice.currency)}</td>
        </tr>
      `,
      )
      .join("");
...
<body>
  <main class="doc">
    <section class="top">
      ...
      <div class="meta">
        ...
        <div><strong>N°:</strong> ${escapeHtml(invoice.invoice_number)}</div>
        <div><strong>Date emission:</strong> ${formatDate(invoice.issue_date)}</div>
        <div><strong>Echeance:</strong> ${formatDate(invoice.due_date)}</div>
        <div><strong>Statut:</strong> ${escapeHtml(invoice.status)}</div>
      </div>
    </section>
    ...
    <table class="totals">
      <tr><td class="label">Sous-total</td><td class="num">${formatCurrency(Number(invoice.subtotal ?? 0), invoice.currency)}</td></tr>
      <tr><td class="label">Remise</td><td class="num">-${formatCurrency(Number(invoice.discount_amount ?? 0), invoice.currency)}</td></tr>
      <tr><td class="label">TVA (${Number(invoice.tax_rate ?? 0).toFixed(2)}%)</td><td class="num">${formatCurrency(Number(invoice.tax_amount ?? 0), invoice.currency)}</td></tr>
      <tr class="grand"><td class="label">Total TTC</td><td class="num">${formatCurrency(Number(invoice.total_amount ?? 0), invoice.currency)}</td></tr>
      <tr><td class="label">Deja regle</td><td class="num">${formatCurrency(Number(invoice.paid_amount ?? 0), invoice.currency)}</td></tr>
      <tr class="due"><td class="label">Reste a payer</td><td class="num">${formatCurrency(Number(invoice.balance_amount ?? 0), invoice.currency)}</td></tr>
    </table>

    ${
      invoice.notes
        ? `<section class="box" style="margin-top: 12px;"><h3>Notes</h3><div class="muted">${escapeHtml(invoice.notes).replaceAll("\n", "<br/>")}</div></section>`
        : ""
    }

    <footer class="footer">
      Document numerote automatiquement. Facture emise par ${escapeHtml(conciergeName)}.
      <br/>${vatMention}
    </footer>
  </main>
  ${autoPrint ? `<script>window.addEventListener('load', () => window.print());</script>` : ""}
    ...
    console.error("[GET /api/invoices/:id/document] ERROR:", err);
