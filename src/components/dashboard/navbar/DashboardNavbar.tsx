...
export default function DashboardNavbar({
  ...
  useEffect(() => {
    ...
    async function loadNotifications() {
      try {
        ...
        let workflowCount = 0;
        let workflowItems: NotificationItem[] = [];

        if (userRole === "owner" || userRole === "owner_pro") {
          const [messageResponse, requestResponse, workflowResponse] = await Promise.all([
            ...
            fetch("/api/notifications?limit=10&unreadOnly=1", { cache: "no-store" }),
          ]);
          const [messagePayload, requestPayload, workflowPayload] = await Promise.all([
          ...
          if (workflowResponse.ok) {
            const rows: WorkflowNotificationRow[] = Array.isArray(workflowPayload?.items)
              ? workflowPayload.items
              : [];
            workflowCount = Number(workflowPayload?.summary?.unread ?? rows.length);
            workflowItems = rows.slice(0, 6).map((item) => ({
              id: `workflow-${item.id}`,
              title: item.title || "Nouvelle notification",
              description: item.body || "Un evenement important a eu lieu sur votre dossier.",
              href: item.action_url || notificationsPageHref,
              count: 1,
              kind: getWorkflowNotificationKind(item.entity_type, item.notification_type),
            }));
          }

          nextCount = messageCount + requestCount + workflowCount;
          nextItems = [...workflowItems, ...requestItems, ...messageItems]
          ...
          if (workflowResponse.ok) {
            ...
            workflowItems = rows.slice(0, 6).map((item) => ({
              id: `workflow-${item.id}`,
              title: item.title || "Nouvelle notification",
              description: item.body || "Un evenement important a eu lieu sur votre activite.",
              href: item.action_url || notificationsPageHref,
              count: 1,
              kind: getWorkflowNotificationKind(item.entity_type, item.notification_type),
            }));
  ...
  }, [isAuthenticated, notificationCount, notificationsPageHref, refreshTick, userRole]);
  ...
  const handleNotificationItemClick = useCallback((item: NotificationItem) => {
    if (item.id.startsWith("workflow-")) {
      ...
      void fetch(`/api/notifications/${workflowId}/read`, {
        method: "PATCH",
      });
    }
