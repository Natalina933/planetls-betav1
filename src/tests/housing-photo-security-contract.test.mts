...
test("uploaded JPG paths render through the authenticated photo route without the image optimizer", () => {
  const path = "4dae1e64-5d5e-4ab7-ad7f-7eed4b3e46c9/30/1789231396498-r8f865.JPG";
  const src = toHousingPhotoUrl(path, 30);
  const { props } = getImageProps({ src, alt: "Logement", width: 220, height: 180, unoptimized: true });
  ...
  assert.equal(toHousingPhotoUrl("/images/default-logement.png", 30), "/images/default-logement.png");
  assert.equal(toHousingPhotoUrl("https://cdn.test/photo.jpg", 30), "https://cdn.test/photo.jpg");
});
...
test("dashboard and housing cards resolve private photos and preserve browser authentication", () => {
  ...
  const conciergeDetail = read("../app/components/dashboard/concierge/LogementPage.tsx");
  const avatar = read("../components/ui/Avatar/Avatar.tsx");
  assert.match(dashboard, /src=\{toHousingPhotoUrl\(property\.photo_principale, property\.id\)\} unoptimized/);
  assert.equal([...list.matchAll(/src=\{toHousingPhotoUrl\(getSafePhoto\(logement\.photo_principale\), logement\.id\)\} unoptimized/g)].length, 2);
  assert.match(ownerDetail, /src=\{draft\.photo_principale \? toHousingPhotoUrl\(draft\.photo_principale, id\) : null\}/);
  assert.match(conciergeDetail, /src=\{draft\.photo_principale \? toHousingPhotoUrl\(draft\.photo_principale, id\) : null\}/);
