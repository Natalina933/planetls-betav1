...
test("admin : les KPI activation utilisent le schéma connecté et exposent leurs seuils", async ({ page }) => {
  await loginAdmin(page);
  const response = await page.request.get("/api/kpis/overview?window_days=30");
  expect(response.ok(), await response.text()).toBeTruthy();
  const payload = (await response.json()) as { activation_alert_policy?: { minimum_eligible?: number }; activation_alerts?: unknown[] };
