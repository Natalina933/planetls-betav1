...
| Dashboard administrateur | 🟡 En cours | N3 | Mission Control admin recentré sur priorités, activité, tables métier, graphiques d'activation, donuts de répartition/contrôle, cartes de santé visuelles par section, hero éditorial premium et filtres segment/période ; la page `controle` suit maintenant le même niveau premium avec hero santé, onglets décisionnels et surfaces de pilotage plus lisibles ; responsive mobile des tableaux et accessibilité clavier/lecteur d'écran renforcées ; overview, contrôle, utilisateurs et vues par rôle reliés ; validations connectées et navigation E2E encore à renforcer |
...
| KPI produit | 🟡 En cours | N3 | Endpoint overview et affichage admin ; activation J+7, temps de première valeur, conversion et séries fiables disponibles ; en local, `/api/kpis/overview` injecte désormais des cohortes workspace crédibles quand Supabase est inaccessible ou quand aucune cohorte mature n'existe encore, afin d'éviter les graphiques vides et les `Donnée insuffisante` permanents pendant l'amorçage |
...
| E2E owner complet | ⚠️ Bloqué | P0 Critique | Court terme | QA + Produit | Demande → devis → mission → facture payée par webhook signé PASS ; scénario Checkout hébergé, carte test, retour owner et sync prêt ; aucune E2E_STRIPE_SECRET_KEY locale disponible pour la preuve finale |
...
| Qualité | E2E transactionnel commercial | Owner, concierge | ⚠️ Bloqué | P0 Critique | 2026-07-19 | Parcours métier et webhook signé 1/1 PASS ; branche Checkout Stripe hébergée prête avec garde sk_test_, carte de test, retour facture et synchronisation ; clé absente localement | Ajouter E2E_STRIPE_SECRET_KEY de test puis exécuter le scénario pour déplacer le statut à Terminé |
