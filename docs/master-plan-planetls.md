...
| Administration | Centre de santé opérationnelle | Admin | ✅ Terminé | P0 Critique | 2026-07-27 | État global sur 12 sources, sources non vérifiables explicites et cycle persistant : prise en charge, transmission au responsable et clôture avec compte rendu, sans masquer l’anomalie ; la page `/dashboard/admin/controle` a été allégée en supprimant la synthèse KPI dupliquée, en ajoutant des panneaux repliables alignés sur la page Développement et en condensant les cartes `Inscriptions`, `Missions` et `Messages` autour de repères plus courts ; la page `/dashboard/admin` a aussi été resserrée avec sections repliables, fusion du parcours avec les feux de contrôle, réduction des accès métier redondants et refonte premium de `Lecture visuelle` sans illustrations, avec rubans de métriques, légendes enrichies et états zéro plus lisibles ; la route `/api/admin/control-tower` tolère désormais l’indisponibilité transport de Supabase via un mode dégradé traçable, garde 12 sources lisibles, injecte une anomalie de repli si tout le diagnostic distant est hors ligne et persiste localement les actions admin pour ne plus renvoyer `500` dans cet environnement ; vérifications : ESLint ciblé PASS, Playwright `/dashboard/admin` `1/1 PASS`, Playwright `admin-control-actions` `1/1 PASS` | Corriger séparément les dérives connectées `missions.title` et `onboarding_events.profile_id`, puis rétablir les données connectées complètes quand Supabase redevient joignable pour sortir du mode dégradé |
