...
| Pilotage PlanetLS — cockpit entrepreneurial privé | 2026-07-29 | 2026-07-29 | 🟡 En cours | Une première page `/dashboard/admin/pilotage` expose déjà une lecture business `acquisition, activation, pipeline, monétisation, tension de facturation`, mais le module complet persistant reste à construire ; absence actuelle de tables dédiées pour budget, transactions, trésorerie, provisions, réserve, objectifs entrepreneuriaux, journal, risques et décisions business ; les montants affichés aujourd'hui sont des estimations issues des données opérationnelles visibles et ne doivent pas être lus comme une comptabilité canonique | Endpoints admin existants, futurs paramètres financiers configurables, Supabase, permissions admin strictes, Stripe si branché, module KPI | Construire par lots le socle persistant et sécurisé : 1) paramètres + permissions, 2) budget prévisionnel + registre recettes/dépenses, 3) trésorerie + provisions + réserve, 4) KPI SaaS + revenus + coûts techniques/IA, 5) objectifs + roadmap + journal + risques + décisions, sans casser le cockpit de lecture actuel |
