...
| Dashboard administrateur | 🟡 En cours | N3 | Mission Control admin recentré sur priorités, activité, tables métier, graphiques d'activation, donuts de répartition/contrôle, cartes de santé visuelles par section, hero éditorial premium et filtres segment/période ; la page `controle` suit maintenant le même niveau premium avec hero santé, onglets décisionnels et surfaces de pilotage plus lisibles ; le parseur d'actions admin accepte aussi des identifiants système stables non UUID pour préparer le suivi d'incidents transverses sans casser les cibles métier existantes ; la page `developpement` embarque désormais un `Conseiller projet` qui répond à des questions de pilotage à partir du Master Plan, de la roadmap, de Mission Control, de la mémoire technique et de scans heuristiques du repo ; la page `/dashboard/admin/pilotage` synthétise désormais acquisition, activation, pipeline missions, conversion de facturation, tensions business et actions recommandées à partir des endpoints admin existants, affiche une lecture financière plus directive `prix, abonnement, commission, réserve solidaire`, et conserve maintenant un bloc `Due diligence investisseur` avec verdict, scores, questions critiques, red flags et conditions avant levée ; lecture dégradée maintenue quand certaines sources sont indisponibles ; responsive mobile des tableaux et accessibilité clavier/lecteur d'écran renforcées ; overview, contrôle, pilotage, utilisateurs et vues par rôle reliés ; validations connectées et navigation E2E encore à renforcer |
...
| Pilotage PlanetLS — cockpit entrepreneurial privé | 2026-07-29 | 2026-08-03 | 🟠 Partiel | Le cockpit opérationnel reste en place mais le centre de stratégie business de la page `/dashboard/admin/pilotage` n'est plus présenté comme un comparateur. Il expose désormais un cadrage financier direct : recommandation `Conciergerie Pro` à `99 € HT / mois` en lancement puis `149 € HT / mois` en cible, ordre de grandeur de revenus selon le nombre d'abonnés, hypothèse de commission à `8 %` si PlanetLS choisit une intermédiation sur missions, rappel que l'idée d'une part solidaire associations doit rester différée jusqu'au choix économique principal, et bloc `Due diligence investisseur` avec verdict, scores, red flags et conditions avant levée. Les exports PDF/Excel/CSV/Business Plan/Pitch et l analyse IA restent désactivés, et le modèle Stripe Concierge PRO existant n est pas modifié. | Composants UI partagés, navigateur, future persistance Supabase admin, futurs paramètres financiers canoniques, Stripe si une stratégie est validée | P1 Prioritaire : choisir d'abord entre `abonnement prioritaire` et `commission`, tester le prix `99 € HT / mois` et l'acceptabilité d'une commission `8 %` sur prospects réels, puis seulement traduire le modèle retenu dans Stripe et la persistance admin |
...
### Mise Ã  jour ciblÃ©e â€” Pilotage Business du lundi 3 aoÃ»t 2026

- Statut : `ðŸŸ  Partiel`
- PrioritÃ© : `P1 Prioritaire`
- PÃ©rimÃ¨tre mis Ã  jour : `/dashboard/admin/pilotage`
- RÃ©alitÃ© produit : le haut de page a Ã©tÃ© simplifiÃ© pour retirer les blocs exploratoires devenus encombrants (`comparateur d'offre Pro`, `memo investisseur`) et garder un cockpit plus directement exploitable.
- Ajout majeur : un premier `RiskRegister` statique et filtrable est maintenant affichÃ© dans la page via `src/app/dashboard/admin/pilotage/risk-register/`, sans migration ni persistance Supabase.
- Source d'alimentation : les risques affichÃ©s reprennent la lecture due diligence et la transforment en cartographie opÃ©rationnelle par prioritÃ©, catÃ©gorie, profils impactÃ©s, horizon, mitigation et signaux d'alerte.
- Limites connues : le registre ne permet encore ni Ã©dition, ni assignation persistÃ©e, ni historique, ni scoring dynamique ; il ne remplace pas encore un vrai module de gouvernance des risques.
- DÃ©pendances inchangÃ©es : endpoints admin existants, composants dashboard partagÃ©s, future persistance Supabase admin, futur arbitrage Stripe si un modÃ¨le financier est figÃ©.
- DÃ©cision de pilotage : conserver le cadrage financier et la synthÃ¨se investisseur dans ce Master Plan comme base d'arbitrage, sans les laisser occuper la page opÃ©rationnelle tant que le modÃ¨le Ã©conomique n'est pas validÃ©.
- Preuve de vÃ©rification : `npm run build` PASS le lundi 3 aoÃ»t 2026.
- Prochaine Ã©tape recommandÃ©e : dÃ©finir plus tard un schÃ©ma canonique `risk_register` seulement aprÃ¨s validation du noyau business `offre, prix, commission ou non`.

Ajout du lundi 3 aoÃ»t 2026 : la vue rapide chiffrÃ©e du haut de page `Pilotage Business` a aussi Ã©tÃ© retirÃ©e, car les tuiles `pipeline`, `missions facturÃ©es`, `activation moyenne` et `points de vigilance` donnaient une prÃ©cision trompeuse. Le haut de page est maintenant recentrÃ© sur un cadrage dÃ©cisionnel qualitatif.
Ajout du lundi 3 aoÃ»t 2026 : la page `Pilotage Business` embarque maintenant un module `Conseiller stratÃ©gique` statique qui formalise la mÃ©thode de dÃ©cision de la fondatrice en 8 Ã©tapes, les questions stratÃ©giques Ã  poser, les critÃ¨res de comparaison des options et les sorties de pilotage Ã  historiser. Cette brique reste volontairement sans persistance ni automatisation afin de prÃ©server la simplicitÃ© tout en crÃ©ant une mÃ©moire de travail rÃ©utilisable.
Ajout du lundi 3 aoÃ»t 2026 : la page `Pilotage Business` expose aussi un module `Validation marchÃ©` statique de type Lean Startup. Il consolide un diagnostic initial, le classement des hypothÃ¨ses critiques, un plan de validation sur 30 jours, les 13 tests mesurables, les scripts d'entretien, le sondage, les variantes de landing, les KPI de validation, la grille `GO / TEST MORE / PIVOT`, les recommandations d'intÃ©gration et les actions immÃ©diates. Cette brique s'appuie sur les parcours dÃ©jÃ  disponibles, ne modifie pas Stripe, ne lance aucune migration et sert d'abord de cadre d'exÃ©cution frugal avant toute nouvelle couche produit.
